...
class item : public crv_sequence_item {
 public:
  item(crv_object_name) : _i() {
    ...
    c_dest_conditions = {
        dest_addr_vec().size() == tmp(), foreach (dest_addr_vec(), if_then(_i == 0, dest_addr_vec()[_i] < 0xF)),
        foreach (dest_addr_vec(), dest_addr_vec()[_i] == dest_addr_vec()[_i - 1] + 8), unique(dest_addr_vec())};

    c_data_conditions = {data_vec().size() == 2 * tmp(),
                         foreach (data_vec(), if_then_else(_i % 2 == 0, data_vec()[_i] <= 10,
  ...
  crv_vector<uint> src_addr_vec{"src_addr_vec"};
  crv_vector<uint> dest_addr_vec{"dest_addr_vec"};
